...
def erro(params):
    ...
    t_eval = target_antibody_data[:, 0] 
   
    #interpolate model onto experimental times
    model_t, solution = model(params, t_eval)
    model_solution = solution.T
    model_antibody = model_solution[:,11]
    ...
    model_antibody_interp = interp1d(model_t, model_antibody, kind = 'linear', fill_value='extrapolate')
