...
beta_ap = 1e-2  # Maturation rate of APCs
...
beta_ps = 1.5e-5  # Differentiation rate of active B cells into short-lived plasma cells
...
def f(t,y,dspline_x,spline_x):
    dydt = np.zeros(eq)
    dydt[0] = 0 if t > 54 else (dspline_x(t))
    dydt[1] = alpha_ap * (Ap0 - y[1]) - beta_ap * c_ap1 * (y[1] * y[0] + y[1] * c_ap2 * y[0]**(c_ap2-1) * dydt[0] * ((c_ap3**c_ap2)/(c_ap3**c_ap2 + c_ap2)**2))
    dydt[2] = beta_ap * c_ap1 * (y[1] * y[0] + y[1] * c_ap2 * y[0]**(c_ap2-1) * dydt[0] * ((c_ap3**c_ap2)/(c_ap3**c_ap2 + c_ap2)**2)) - delta_apm * y[2]
...
c_ap3 = spline_x(t_eval).max()*1.8
# Solve the system using solve_ivp
solution = solve_ivp(f, [t0, t_final], y0, method='LSODA', t_eval=t_eval, args=(spline_x.derivative(1),spline_x))
...
#t_plot = np.linspace(told.min(), told.max(), 1000)
#plt.figure(figsize=(8, 5))
#plt.plot(t_plot, spline_x(t_plot), label='Spline curve')
#plt.plot(told, xold, 'ro', label='Original points')
#plt.grid(True)
#plt.legend()
#plt.title('Spline vs Original Viremia')
#plt.show()
