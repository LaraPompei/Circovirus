...
#define k_v3 1.05e-7     //Texa de eliminaçao do virus por unidade do sistema imune inato
...
#define delta_l 1.8e-1 //taca de decaimento natural das celulas do sistema imune inato
...
#define beta_tk 0.7e-5        //Taxa de ativacao das celulas T CD8+ naive
...
#define delta_tk 5e-4       //Taxa de morte das celulas T CD8+ efetoras
...
#define pi_b1 4.83e-9       //Taxa de ativacao das celulas B T-independente
...
#define delta_ps 2.02        //Taxa de morte dos plasmocitos de vida curta
...
void Sistema(double *y, double* dydt){
    //V
    dydt[0] = pi_v*y[0]-  10*k_v1*y[0]*y[11] - k_v3*y[0]*y[13];//pi_v*y[0]-  k_v1*y[0]*y[11] - k_v2*y[0]*y[6] - k_v3*y[0]*y[13]; 
    //- (c_v1*y[0])/(c_v2+y[0]);//- k_v3*y[0]*y[13];
    //Ap
    dydt[1] = 0;//alpha_ap*(Ap0 - y[1]) - beta_ap*y[1]*(c_ap1*(y[0])/(c_ap2 + y[0]));
    //Apm
    dydt[2] = 0;//beta_ap*y[1]*(c_ap1*(y[0])/(c_ap2 + y[0])) - delta_apm*y[2];
    //Thn
    dydt[3] = 0;//alpha_th*(Thn0 - y[3]) - beta_th*y[2]*y[3];
    //The
    dydt[4] = 0;//beta_th*y[2]*y[3] + pi_th*y[2]*y[4] - delta_th*y[4];
    //Tkn
    dydt[5] = 0;//alpha_tk*(Tkn0 - y[5]) - beta_tk*y[2]*y[5];
    //Tke
    dydt[6] = 0;//beta_tk*y[2]*y[5] + pi_tk*y[2]*y[6] - delta_tk*y[6];
    //B
    //dydt[7] = alpha_b*(B0 - y[7]) + pi_b2*y[4]*y[7] - beta_ps*y[2]*y[7] - beta_pl*y[4]*y[7] - beta_bm*y[4]*y[7];
    dydt[7] = alpha_b*(B0 - y[7]) + pi_b2*y[0]*y[7] - beta_bm*y[0]*y[7];
    //Ps
    //dydt[8] = beta_ps*y[2]*y[7] - delta_ps*y[8];
     dydt[8] = 0;//beta_ps*y[7] - delta_ps*y[8];
    //Pl
    dydt[9] = 0;//beta_pl*y[4]*y[7] - delta_pl*y[9] + gama_bm*y[10];
    //Bm
    dydt[10] = beta_bm*y[0]*y[7] + pi_bm1*y[10]*(1 - (y[10]/pi_bm2)) - gama_bm*y[10];
    //IgM
    //dydt[11] = pi_ps*y[8] - delta_IgM*y[11];
//    dydt[11] = c_ps1*(1-exp(pow(-(y[8]/c_ps2),c_ps3))*exp(pow((-y[8]/c_ps4),c_ps5))) - delta_IgM*y[11];
    dydt[11] = c_ps1 * pow(y[7],c_ps2) / (pow(c_ps3,c_ps2) + pow(y[7],c_ps2)) - delta_IgM * y[11];
    //IgG
    //dydt[12] = pi_pl*y[9] - delta_IgG*y[12];
    dydt[12] = pi_pl*y[10] - delta_IgG*y[12];
    //L
...
*/
    ...
    outputFile<<"t,V,Ap,Apm,Thn,The,Tkn,Tke,B,Ps,Pl,Bm,IgM,IgG,L"<<endl;
    outputFile.close();
    RK5(t,h,y,inter);
