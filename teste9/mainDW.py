...
k1 = 1       # Maximum maturation rate of APCs
k2 = 20      # Half activation constant
k3 = 20     # Day the APm start being produced
...
def f(t,y,spline_x):
    ...
    dydt[1] = alpha_ap * (Ap0 - y[1]) - k1 * y[1] * (1 - np.exp(-(y[0]/k2)**k3)*np.exp(-(y[0]/k4)**k5))
    dydt[2] = k1 * y[1] * (1 - np.exp(-(y[0]/k2)**k3)*np.exp(-(y[0]/k4)**k5)) - delta_apm * y[2]
    ...
    #dydt[7] = alpha_b * (B0 - y[7]) + pi_b2 * y[4] * y[7] - beta_ps * y[2] * (k1 * y[7]**k2 / (k3**k2 + y[7]**k2)) - beta_pl * y[4] * y[7] - beta_bm * y[4] * y[7]
    #dydt[8] = beta_ps * y[2] * (k1 * y[7]**k2 / (k3**k2 + y[7]**k2)) - delta_ps * y[8]
